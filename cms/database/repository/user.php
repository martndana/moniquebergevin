...
class User
{
    ...
    /**
     * Gets all users from the database ordered by name
     *
     * @return array [idUsers, uidUsers, pwdUsers]
     */
    function get(): array
    {
        $query = "
            SELECT idUsers, uidUsers, pwdUsers
            FROM users
            ORDER BY uidUsers
        ";
     ...
     * Gets all users from the database ordered by name
     *
     * @return object {idUsers, uidUsers, pwdUsers}
     */
    ...
    function update(int $id, string  $sUsername, string $sPassword): bool
    {
        try {
            ...
            $query = "
                UPDATE users
                SET uidUSers = :sUsername, pwdUSers = SHA1(:sPassword)
                WHERE idUsers = :id;
            ";
